...
$$
\texttt{Cons(}\underbrace{\texttt{1}}\_{\text{head}}\texttt{,} \underbrace{\texttt{Cons(2, Cons(3, Nil)))}}\_{\text{tail}}
$$
...
Pretty similar, huh? Here, `List<A>` takes the place of `'a list`. `A`, like
...
[The macro][rust-list-macro] lets us write things like `list![]`, `list![1i]`,
...
This type signature should look very similar to our OCaml type where `A` and
`B` are substitutes for `'b` and `'a` respectively. Now, let's add the body.
...
empty we evaluate to the accumulator. When the list is non-empty, we compute a
new accumulator using `f` and then recurse.

You can play with `fold_left` yourself!

<div style="height: 775px; overflow: hidden"><iframe src="http://is.gd/Cuvy7b" frameborder="0" border="0" cellspacing="0" style="border-style: none;width: 100%; height: 1100px;"></iframe></div>

## 3.4 `map` ##
Now that we have `fold_left` written, let's write a simpler function, `map`,
...
`map f [a1, ..., an]` evaluates to `[f a1, ..., f an]`. For example `map (fun x
-> x + 1) [1; 2; 3]` evaluates to `[2; 3; 4]`.

What's neat about `map` is that we can implement it using `fold_left` (and a
function `reved` which reverses a list)!
...
So far, so good. We've written `fold_left` and used it to implement `map`. Now,
let's experiment with `map` to make sure it works how it should. First, let's
create a list of integers `xs`. Then, we'll map the identity function `|x| x`
across the list. Since `map` expects a function of type `|&A| -> B`, this
should create of int references (`List<&int>`) from `xs`.
...
So what happens when we run this program? Well, actually it doesn't even compile.

```
list.rs:119:37: 119:38 error: cannot infer an appropriate lifetime due to conflicting requirements
list.rs:119     let ys: List<&int> = xs.map(|x| x);
                                                ^
list.rs:119:37: 119:38 note: first, the lifetime cannot outlive an anonymous lifetime defined on the block at 119:36...
list.rs:119     let ys: List<&int> = xs.map(|x| x);
                                                ^
list.rs:119:37: 119:38 note: ...so that expression is assignable (expected `&int`, found `&int`)
list.rs:119     let ys: List<&int> = xs.map(|x| x);
                                                ^
list.rs:119:26: 119:39 note: but, the lifetime must be valid for the method call at 119:25...
list.rs:119     let ys: List<&int> = xs.map(|x| x);
                                     ^~~~~~~~~~~~~
list.rs:119:26: 119:39 note: ...so that the declared lifetime parameter bounds are satisfied
list.rs:119     let ys: List<&int> = xs.map(|x| x);
                                     ^~~~~~~~~~~~~
error: aborting due to previous error
...
So what's going on here? Well, the compiler is telling us that it can't infer a
proper lifetime. It says the lifetime of `x` is limited to the lifetime of the
closure `|x| x`, but we want the lifetime of `x` to be the same as the lifetime
of `xs`.

Wow.

That's an interesting bug that you won't find in pretty much any other
programming language besides Rust and one that I admittedly didn't even know
existed until I had written a fairly large chunk of the code. Luckily, it turns
out that the fix is pretty simple; we just have to explicitly annotate the
