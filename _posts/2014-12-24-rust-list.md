...
-> x + 1) [1; 2; 3]` evaluates to `[2; 3; 4]`, and `map (fun x -> x * x) [1; 2;
3]` evaluates to `[1; 4; 9]`.
...
let's experiment with `map` to make sure it works how we think it should.
First, let's create a list of integers `xs`. Then, we'll map the identity
function `|x| x` across the list. Since `map` expects a function of type `|&A|
-> B`, this should create a list of int references (`List<&int>`) from `xs`.
...
So what happens when we [run this program](http://is.gd/sMUCqs)? Well, click
...
existed until I had written a fairly large chunk of the code. I'll again defer
an in-depth explanation to [elsewhere][example-lifetime], but I'll show you the
fix: we just have to explicitly annotate the lifetimes of our references.
